...
void readFromLine(abiturient* s, char* line) {
    ...
    int ball;
    int offset = 0; // количество считанных символов

    sscanf(line, "%s %s %s %d %s %s %d%n", s->surname, s->initials, s->gender, &(s->schoolNum), s->medal, s->composition, &(s->numExams), &offset); // прочитали часть строки
    line += offset; // подвинули указатель туда, гле остановились
    int num = s->numExams;
...
void addAbiturient(const char* filename, char* in, HashTable* table) {
    ...
    fseek(newFile, 0, SEEK_END); // смещаем указатель на конец файла
    fseek(newFile, -1, SEEK_CUR); // а теперь на единицу влево
    char lastChar = fgetc(newFile);

    if(lastChar == '\n' || ftell(newFile) == 0) { // или переноси, или вообще пустой
        rewind(newFile);
