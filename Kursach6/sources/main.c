...
void printMenu() {
    ...
    printf("| find - найти абитуриентов-медалистов, не набравших проходной балл\n");
    printf("| table - распечатать таблицу\n");
    printf("| remove - удаление абитуриента(remove [Фамилия] [Инициалы])\n");
}

//функция поиска студента по условию
void HaveNoMinBall(HashTable* table, float passBall) {
...
int main(int argc, char *argv[]) {
    ...
    while(strcmp(choose, "exit") != 0) { 
        ...
        } else if (strcmp(cmd, "find") == 0) { // задание
            printf("Введите проходной балл: ");
            scanf("%f", &ball);
            getchar(); //чтобы не вылетало после выполнения HaveNoMinBall(программа обрабатывает число, потому считает его в цикле за команду)
            HaveNoMinBall(table, ball);
        } else { 
            continue;
        }
